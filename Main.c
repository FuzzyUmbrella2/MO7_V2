...
u8 is_sample_ready = 0;	// unsigned 8 bit integer used as a boolean to indicate if new audio sample is available from I2S
...
void loop()
{
	while (is_sample_ready == 0) {
		...
		is_sample_ready = (status_reg >> 21 ) & 1;
	}
	is_sample_ready = 0;
